"""Independent adaptive Dormand--Prince 5(4) integration for six-state orbits.

This module is deliberately separate from the project's RK4 propagators.  It
is an independent integration algorithm that can share a force callback for a
numerical cross-check; it does not provide an independent physical model.

The tableau and controller follow the public SciPy RK45 implementation as an
algorithm reference (no SciPy dependency is used here):
https://github.com/scipy/scipy/blob/main/scipy/integrate/_ivp/rk.py

Positions and velocities are kept as a flat six-component state.  ``epoch``
and ``output_times`` use the caller's time units and are absolute times, while
the accepted-step loop uses local time ``x = t - epoch``.  The RHS receives
``epoch + x`` so a time-dependent force still sees the requested epoch.
"""
...
def integrate_dopri54(
    ...
    *,
    ...
    """Integrate a six-component position/velocity state with adaptive DP5(4).
    ...
    while target_index < len(targets):
        ...
        for stage in range(1, 7):
            stage_state = _add_scaled(current, tuple(derivatives), _A[stage], step)
            derivatives.append(_rhs(acceleration, epoch_value + local_time + _C[stage] * step, stage_state))
        function_evaluations += 6
        high = _add_scaled(current, tuple(derivatives), _B, step)
        error = tuple(step * sum(coefficient * derivative[index] for coefficient, derivative in zip(_E, derivatives)) for index in range(6))  # type: ignore[assignment]
        ...
        if error_value <= 1.0:
            ...
            local_time = next_boundary if reaches_boundary else local_time + step
            current = high
            accepted_steps += 1
...
__all__ = ["Acceleration", "IntegrationResult", "Sample", "State6", "integrate_dopri54"]
