"""Read only the required DE441 binary ranges; never download the full kernel."""
...
CONFIG = 'configs/apophis_spk_inputs.json'
...
def prepare(root, download=False):
    ...
    hashes = {p: sha(root / p) for p in [CONFIG, 'src/prepare_apophis_spk.py']}
    if design.exists():
