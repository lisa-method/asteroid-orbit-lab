"""Frozen selector-v4 evaluation on the whole-object holdout24 sample.
...
from prepare_selector_v4_holdout24 import MANIFEST_PATH, RAW_DIRECTORY, SAMPLE_PATH, check_hashes
...
METHOD_FREEZE = "outputs/selector_v4/method_freeze.json"
...
def _case(obj: Mapping[str, Any], model: Mapping[str, Any], horizon: float, context: Mapping[str, Any],
    ...
    record = _record_for_horizon(initial, start, horizon, model, context, config, reference,
                                 ...
                                 str(obj["start_date"]), "selector_v4_holdout24")
    record["evaluation_scope"] = "frozen_selector_v4_new_holdout24"
...
def _provenance(root: Path, sample: Mapping[str, Any], manifest: Mapping[str, Any]) -> dict[str, Any]:
    return {"scope": "selector-v4 whole-object holdout24", "source_hashes": source_hashes(root, sample, manifest),
...
def matrix(root: Path, *, resume: bool = False) -> dict[str, Any]:
    ...
    result = {"schema_version": 1, "experiment": "selector_v4_holdout24", "provenance": provenance,
