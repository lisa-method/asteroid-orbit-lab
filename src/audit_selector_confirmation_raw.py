"""Offline audit of every raw manifest and the 156 new target tables."""
import argparse
...
from prepare_selector_confirmation import (
    MANIFEST as MANIFEST_PATH, SAMPLE as SAMPLE_PATH, PARENT as METHOD_FREEZE,
    jobs as build_selector_jobs, validate as validate_downloads,
    check_hashes, immutable as immutable_json, sha,
)
...
def audit(root):
    ...
    experiment_path = root / "outputs/selector_confirmation100/experiment_freeze.json"
    experiment = json.loads(experiment_path.read_text())
    check_hashes(root, experiment['hashes'])
    if manifest.get('experiment_freeze_sha256') != sha(experiment_path):
        raise ValueError('Confirmation experiment freeze differs')
    jobs = build_selector_jobs(root, sample)
    if len(jobs) != len(manifest["files"]) or len(jobs) != 156:
        raise ValueError("Require precisely 156 new target tables")
    ...
    if len(paths) != 684 or len(manifests) != 23 or validated["shared_daily_refined_nodes"] != 280:
    ...
    value = {"passed": True, "unique_raw_paths": len(paths), "manifest_count": len(manifests),
             ...
             "shared_daily_refined_nodes": 280, "ng_status_counts": dict(Counter(v["status"] for v in validated["ng"].values())),
    ...
    immutable_json(root / "outputs/selector_confirmation100/raw_verification.json", value)
