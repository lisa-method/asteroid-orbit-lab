"""Bounded Apophis Moon/Venus ephemeris input audit (no training or selector run)."""
...
def _normalize_trace(meta: dict, start: float, initial: State, solver: str) -> list[list[Any]]:
    ...
    if solver == "rk4":
        trace = [[start + row[0], row[1]] for row in trace]
    initial_flat = _state6(_flat(initial))
    if not trace or trace[0][0] != start:
        trace.insert(0, [start, initial_flat])
...
def _module_paths(root: Path) -> list[Path]:
    ...
    import ast
    queue = ["run_apophis_moon_venus", "prepare_apophis_moon_venus", "finalize_apophis_moon_venus_inputs"]
    visited = set()
    while queue:
        name = queue.pop()
...
def _run_variant(case: str, arm: dict, solver: str, scale_or_tol: Any, target_rows: list[dict], dense_rows: list[dict], planets: tuple[Perturber, ...], ng: Any, config: dict, constants: dict, out: Path, fingerprint: str, baseline: dict[str, float] | None) -> dict:
    ...
    if accepted[-1][0] < target_rows[-1]["epoch_jd_tdb"] - 1e-12:
        raise ValueError("accepted trace does not cover the final requested stop")
    dense_predictions = _hermite(accepted, dense_times)
    ...
    requested_hermite = _hermite(accepted, [row["epoch_jd_tdb"] for row in target_rows])
    ...
    result = {"case": case, "arm": arm["id"], "solver": solver, "setting": scale_or_tol, "requested_samples": len(predictions), "dense_samples": len(dense_predictions), "summary_old_grid": _summary(predictions, truth, au_km, day_s), "summary_dense": _summary(dense_predictions, dense_truth, au_km, day_s), "runtime_seconds": meta.get("runtime_seconds"), "counts": {key: value for key, value in meta.items() if key.endswith("steps") or key.endswith("evaluations") or key in {"accepted_steps", "rejected_steps", "attempted_steps"}}, "requested_hermite_consistency": consistency, "requested_states": [list(_flat(state)) for state in predictions], "dense_states": [list(_flat(state)) for state in dense_predictions], "accepted_endpoints": accepted}
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/apophis_moon_venus.json"))
