"""Freeze and fetch the selector-v4 whole-object holdout.
...
the ``selector_v4_holdout24`` namespace.  No forecast error or future target
...
SAMPLE_PATH = "data/processed/selector_v4_holdout24/sample.json"
MANIFEST_PATH = "data/checksums/selector_v4_holdout24_manifest.json"
RAW_DIRECTORY = "data/raw/selector_v4_holdout24"
SEED = "selector-v4-holdout24-2026-09-09"
...
def _excluded_ids(root: Path) -> set[str]:
    ...
    old_selector_sample = json.loads((root / "data/processed/selector_holdout24/sample.json").read_text(encoding="utf-8"))
    ...
    excluded.update(str(obj["id"]) for obj in old_selector_sample.get("objects", []))
    excluded.update(str(value) for value in old_selector_sample.get("excluded_object_ids", []))
...
def _method(root: Path) -> tuple[dict[str, Any], str]:
    ...
    expected_runtime = method.get("runtime")
    actual_runtime = {"python_version": sys.version, "python_implementation": sys.implementation.name,
...
def freeze(root: Path) -> dict[str, Any]:
    ...
    sample = {
        "schema_version": 1,
        "seed": SEED,
        "scope": "selector-v4 whole-object holdout24; metadata-only selection",
        "objects": selected,
        "strata": list(STRATA),
...
def build_selector_jobs(root: Path, sample: dict[str, Any]) -> list[dict[str, Any]]:
    return [job for job in build_jobs(root, _download_config(root), sample)
            if job.get("kind") == "horizons_asteroid"]
...
def _parse_rows(path: Path, target_id: str, target_name: str) -> list[dict[str, Any]]:
    document = _check_header(path)
    header = str(document.get("result", "")).split("$$SOE", 1)[0]
    if not re.search(r"^Target body name:\s*" + re.escape(str(target_id)) + r"\b", header, re.MULTILINE):
        raise ValueError(f"Horizons target identity mismatch: {path}")
    for label in ("Start time", "Stop  time"):
        if not any(line.startswith(label) and line.rstrip().endswith("TDB") for line in header.splitlines()):
            raise ValueError(f"Horizons TDB time contract mismatch: {path}")
