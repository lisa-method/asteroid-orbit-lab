"""Frozen v4 confirmation evaluation on a dynamic whole-object sample.
...
from prepare_selector_confirmation import MANIFEST as MANIFEST_PATH, RAW as RAW_DIRECTORY, SAMPLE as SAMPLE_PATH, PARENT as PARENT_METHOD, check_hashes
...
V4_MODEL = "outputs/selector_v4/model.json"
METHOD_FREEZE = PARENT_METHOD
EXPERIMENT_FREEZE = "outputs/selector_confirmation100/experiment_freeze.json"
...
BODY_IDS = {"Earth": "399", "Venus": "299", "Mars": "4", "Jupiter": "5", "Moon": "301"}
...
def load_inputs(root: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if sample.get("schema_version") != 1 or not isinstance(objects, list) or not objects or len({str(o.get("id")) for o in objects}) != len(objects):
        raise ValueError("sample must contain a nonempty unique object list")
    ...
    if not isinstance(timing_ids, list) or len(timing_ids) != 24 or len(set(map(str, timing_ids))) != 24:
        raise ValueError("sample must contain 24 unique timing_object_ids")
    if not set(map(str, timing_ids)) <= {str(o["id"]) for o in objects}:
        raise ValueError("timing object is absent from sample")
    ...
    if {str(o["id"]) for o in objects} & set(map(str, sample.get("excluded_object_ids", []))):
    ...
    artifact_v3 = json.loads((root / V3_MODEL).read_text())
    artifact_v4 = json.loads((root / V4_MODEL).read_text())
    ...
    if sample.get("method_freeze_sha256") != method_sha:
        raise ValueError("sample/manifest method-freeze hash mismatch")
    experiment = json.loads((root / EXPERIMENT_FREEZE).read_text())
    if experiment.get("runtime") != _runtime_environment():
        raise ValueError("Runtime differs from experiment freeze")
    for relative, expected in experiment.get("hashes", {}).items():
        if str(relative) != SAMPLE_PATH and (not (root / str(relative)).is_file() or sha(root / str(relative)) != str(expected)):
            raise ValueError(f"Experiment freeze dependency changed: {relative}")
    if experiment.get("hashes", {}).get(SAMPLE_PATH) != sha(sample_path):
        raise ValueError("Experiment freeze sample hash mismatch")
    if manifest.get("experiment_freeze_sha256") != sha(root / EXPERIMENT_FREEZE):
        raise ValueError("Manifest experiment-freeze hash mismatch")
    return sample, manifest, v2, {"artifact_v3": artifact_v3, "artifact_v4": artifact_v4,
                                  "rules": rules, "method_freeze": method}


def source_hashes(root: Path, sample: Mapping[str, Any], manifest: Mapping[str, Any]) -> dict[str, str]:
    method = json.loads((root / METHOD_FREEZE).read_text())
    check_hashes(root, method["hashes"])
    relative = [METHOD_FREEZE, EXPERIMENT_FREEZE, SAMPLE_PATH, MANIFEST_PATH]
...
def decide(method: str, feature: Mapping[str, Any], horizon: float, tolerance: float,
            config: Mapping[str, Any], rules: Mapping[str, Any], tree: Mapping[str, Any],
            v4_artifact: Mapping[str, Any] | None = None) -> dict[str, Any]:
    if method not in METHODS:
        raise ValueError(f"unknown selector method: {method}")
    if method == "tree_v3":
        decision = dict(choose_v3(feature, horizon, tolerance, tree))
        decision["no_candidate_predicted"] = not any(cap <= tolerance for cap in decision["predicted_error_caps_km"].values())
    elif method in ("physics_v4", "hybrid_v4"):
        if v4_artifact is None:
            raise ValueError("v4 artifact is required")
        decision = dict(choose_v4(feature, horizon, tolerance, v4_artifact, method=method))
        ...
        raise ValueError(f"unknown selector method: {method}")
    decision.update(method=method, warning=bool(decision.get("warning", False)) or decision.get("status") in ("no_candidate_predicted", "strong_encounter_unvalidated", "outside_training_support"),
...
def forecast_online(
    ...
    artifact_v3: Mapping[str, Any], artifact_v4: Mapping[str, Any], context: Mapping[str, Any],
    ...
    ``fixed_full`` does not pay for feature building.  Each selector method
    builds its own causal features inside this function, which is the
    ...
    if method == "tree_v3":
        ...
        feature = features_v4(initial, start_jd, horizon, dict(context), dict(feature_config), dict(config), ng=ng)
    decision = decide(method, feature, horizon, tolerance, config, {}, artifact_v3, artifact_v4)
    ...
    online = {"runtime_seconds": time.perf_counter() - began, "decision": decision,
              "feature_built": method != "fixed_full",
...
def _event_geometry(obj: Mapping[str, Any], reference: list[dict[str, Any]], start: float,
    ...
    if event is None:
        return None
    body_id = BODY_IDS.get(str(event["body_id"]), str(event["body_id"]))
...
def _case(obj: Mapping[str, Any], model: Mapping[str, Any], horizon: float, context: Mapping[str, Any],
    ...
    record = _record_for_horizon(initial, start, horizon, model, context, config, reference,
                                 ...
                                 str(obj["start_date"]), "selector_confirmation100")
    record["evaluation_scope"] = "frozen_selector_v4_confirmation100"
...
def _run_pair(root: Path, obj: Mapping[str, Any], model: Mapping[str, Any], context: Mapping[str, Any],
    ...
    if feature_cache is None:
        feature_cache = {h: features_v4(initial, start, h, dict(context), dict(feature_config), dict(config), ng=ng) for h in HORIZONS}
    annual = max(HORIZONS)
...
def _choice_rows(records: list[Mapping[str, Any]], sample: Mapping[str, Any], config: Mapping[str, Any], rules: Mapping[str, Any], tree: Mapping[str, Any], v4_artifact: Mapping[str, Any] | None = None) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        for h in HORIZONS:
            ...
            for tol in TOLERANCES:
                for method in METHODS:
                    d = decide(method, feature, h, float(tol), config, rules, tree, v4_artifact)
                    ...
                    choices.append({"object_id": oid, "horizon_days": h, "method": method, "tolerance_km": tol,
                                    ...
                                    "outside_training_support": bool(d.get("outside_training_support", False)),
                                    "outside_training_support_reasons": list(d.get("outside_training_support_reasons", [])),
                                    "warning_reasons": list(d.get("warning_reasons", [])),
                                    "no_candidate_predicted": bool(d.get("no_candidate_predicted", False)),
                                    "rejected_forces": dict(d.get("rejected_forces", {})),
                                    "predicted_error_caps_km": dict(d.get("predicted_error_caps_km", {})),
    ...
    for tol in TOLERANCES:
        for method in METHODS:
            ...
            summaries.append({"tolerance_km": tol, "method": method, "cases": len(subset),
                ...
                "outside_training_support_cases": sum(x["outside_training_support"] for x in subset),
                "no_candidate_predicted_cases": sum(x["no_candidate_predicted"] for x in subset),
                "outside_training_support_reasons": sorted({reason for x in subset for reason in x["outside_training_support_reasons"]}),
...
def _stratum_summaries(choices: list[Mapping[str, Any]], objects: list[Mapping[str, Any]]) -> list[dict[str, Any]]:
    """Summarize every method/tolerance within every sampled stratum."""
    result = []
    strata = sorted({str(obj.get("stratum", "unknown")) for obj in objects})
    for stratum in strata:
        ids = {str(obj["id"]) for obj in objects if str(obj.get("stratum", "unknown")) == stratum}
        for method in METHODS:
            for tolerance in TOLERANCES:
                rows = [r for r in choices if str(r["object_id"]) in ids and r["method"] == method and float(r["tolerance_km"]) == tolerance]
                body_all = sum(all(bool(r["actual_eligible"]) for r in choices if str(r["object_id"]) == oid and r["method"] == method and float(r["tolerance_km"]) == tolerance) for oid in ids)
                result.append({"stratum": stratum, "method": method, "tolerance_km": tolerance,
                               "objects": len(ids), "cases": len(rows), "eligible_cases": sum(bool(r["actual_eligible"]) for r in rows),
                               "body_all_horizons": body_all, "warning_cases": sum(bool(r["warning"]) for r in rows),
                               "unflagged_failures": sum(not bool(r["actual_eligible"]) and not bool(r["warning"]) for r in rows),
                               "worst_position_error_km": max((float(r["max_position_error_km"]) for r in rows), default=0.0)})
    return result


def _provenance(root: Path, sample: Mapping[str, Any], manifest: Mapping[str, Any]) -> dict[str, Any]:
    return {"scope": "selector confirmation100", "source_hashes": source_hashes(root, sample, manifest),
...
def matrix(root: Path, *, resume: bool = False) -> dict[str, Any]:
    ...
    choices, summaries = _choice_rows(records, sample, config, artifacts["rules"], artifacts["artifact_v3"], artifacts["artifact_v4"])
    result = {"schema_version": 1, "experiment": "selector_confirmation100", "provenance": provenance,
              "records": records, "choices": choices, "summaries": summaries,
              "stratum_summaries": _stratum_summaries(choices, sample["objects"]),
...
def direct_cost(root: Path, *, resume: bool = False) -> dict[str, Any]:
    ...
    all_rows = []
    timing_ids = {str(value) for value in sample["timing_object_ids"]}
    for obj in sample["objects"]:
        if str(obj["id"]) not in timing_ids:
            continue
            ...
            for repeat in range(3):
                ...
                for method in order:
                    prediction, metadata, online = forecast_online(
                        initial, start, h, 1.0, method, artifacts["artifact_v3"], artifacts["artifact_v4"],
            ...
            for method in METHODS:
                rows.append({**entries[method], "runtime_trials_seconds": trials[method],
                    ...
                    "includes": ["inference", "force_construction", "rollout"] +
                                (["features"] if method != "fixed_full" else [])})
    ...
    result = {"schema_version": 1, "provenance": provenance, "timings": all_rows, "full_cost": totals,
              ...
              "timing_scope": f"{len(all_rows)} cases x 3 alternating repeats; resident inputs; evaluator and cold loading excluded"}
...
def verify(root: Path) -> dict[str, Any]:
    ...
    provenance = _provenance(root, sample, manifest)
    expected_records = len(sample["objects"]) * len(config["models"]) * len(HORIZONS)
    if result["provenance"] != provenance or result["record_count"] != expected_records:
        raise ValueError("Matrix provenance or count differs")
    context = load_development_context(root, _v2_context_config(config))
    choices, summaries = _choice_rows(result["records"], sample, config, artifacts["rules"], artifacts["artifact_v3"], artifacts["artifact_v4"])
    if choices != result["choices"] or summaries != result["summaries"]:
        raise ValueError("Decisions do not reproduce")
    if result.get("stratum_summaries") != _stratum_summaries(choices, sample["objects"]):
        raise ValueError("Stratum summaries do not reproduce")
    ...
    for row in result["records"]:
        ...
        if h == 365.0 and objects[oid]["event"] is not None:
            ...
            geometry_count += 1
        samples += len(errors)
    expected_traces = len(sample["objects"]) * len(config["models"]) * 2
    expected_geometry = sum(obj.get("event") is not None for obj in sample["objects"]) * len(config["models"])
    if len(unique_paths) != expected_traces or geometry_count != expected_geometry:
        raise ValueError("Incomplete physical traces or event/four-model geometry")
    ...
    expected = {(str(o["id"]), h, m) for o in sample["objects"] for h in HORIZONS for m in METHODS}
    found = {(r["object_id"], r["horizon_days"], r["method"]) for r in timing["timings"]}
    expected_timing = {(str(oid), h, m) for oid in sample["timing_object_ids"] for h in HORIZONS for m in METHODS}
    if len(timing["timings"]) != len(expected_timing) or found != expected_timing:
    ...
    summary = {"passed":True,"records":len(result["records"]),"choices":len(choices),"error_rows":samples,"physical_traces":len(unique_paths),
...
def main(argv: list[str] | None = None) -> int:
    ...
    compact = {k: value[k] for k in ("record_count", "rows", "passed", "records", "choices", "physical_traces", "event_geometry_records", "direct_timing_rows") if k in value}
    if isinstance(compact.get("records"), list):
        compact["records"] = len(compact["records"])
