...
from selector_v3 import features_v3, choose_v3
...
def run(root):
    ...
    artifact = json.loads((root / "outputs/selector_v3/model.json").read_text())
    ...
    for h in config["horizons_days"]:
        feature = features_v3(initial, start, h, context, features_config, ng)
        rows.append({"horizon_days": h, "features": feature,
                     "choices": [choose_v3(feature, h, t, artifact) for t in config["position_tolerances_km"]]})
    value = {"scope": "inspected Apophis warning diagnostic, no fresh score or refit",
             ...
             "reference_sha256": sha(path), "model_sha256": sha(root / "outputs/selector_v3/model.json"),
             "script_sha256": sha(Path(__file__)), "rows": rows}
    immutable_json(root / "outputs/selector_v3/apophis_warning_diagnostic.json", value)
    for r in rows:
