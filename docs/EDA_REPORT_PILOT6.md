...
> Generated 2026-09-04T18:19:32.881869+00:00. This is an exploratory technical pilot, not the final train/validation/test dataset.

## Executive summary

- Parsed **24,108** asteroid states and **36,162** massive-body states.
...
- Correlation between `log10(max eta)` and `log10(|planetary acceleration|)` is **0.7330**; the heliocentric perturbation indicator tracks the omitted-force magnitude as intended.
...
| Raw manifest files | 24 |
| Raw bytes | 12,219,691 |
...
| Massalia | MBA | no | no | 2.40795 | 0.143609 | 0.70934 | 1.0788 | 10321 | 61886 | — |
...
The sample spans Amor, Apollo, Aten, Main-belt Asteroid regimes. It is useful for pipeline stress-testing but is selection-biased and must not become the final evaluation sample.
...
| Massalia | 2.062–2.754 | 16.61–22.18 | 2.4085 | 0.1432 | 1.10e-03 | 7.77e-04 | 0.123/1.87e-06 | 1.1e-08/1.93e-11 | Jupiter system (100.0%) |
...
![Heliocentric trajectories](../figures/eda_pilot_6/orbit_xy.svg)

![Perturbation indicator](../figures/eda_pilot_6/max_eta.svg)

![Earth separation](../figures/eda_pilot_6/earth_distance.svg)
...
![Apophis 2029 refined encounter](../figures/eda_pilot_6/apophis_2029_refined.svg)
...
| 19 | Massalia | Earth | 2022-02-03T00:00:00 | 1.11908 | 167,412,338 | 8.473 | 2.43e-05 | 113 |
| 20 | Massalia | Moon | 2022-02-04T00:00:00 | 1.12138 | 167,756,327 | 7.595 | 2.99e-07 | 494 |
...
2. **The pilot remains selection-biased.** 4 of 6 bodies are NEOs and 3 are PHA objects. Final sampling needs SBDB-based strata across orbit class, eccentricity, inclination, perihelion and encounter intensity.
...
Build a 30-object train/validation pilot using predeclared SBDB strata. Preserve the daily 2020–2030 backbone, refine only candidate encounter windows at hourly and then minute cadence, and reserve whole objects plus complete events before fitting any thresholds. The 6 objects here remain an engineering regression set and should not be reused as the final test set.
...
```bash
env UV_CACHE_DIR=/private/tmp/astrophysics-ml-uv-cache uv run --no-project --python-preference only-system python src/download_jpl_pilot.py --config configs/eda_pilot_6.json --root .
env UV_CACHE_DIR=/private/tmp/astrophysics-ml-uv-cache uv run --no-project --python-preference only-system python src/download_event_refinement.py --base-config configs/eda_pilot_6.json --event-config configs/eda_apophis_2029_refinement.json --root .
env UV_CACHE_DIR=/private/tmp/astrophysics-ml-uv-cache uv run --no-project --python-preference only-system python src/run_eda.py --config configs/eda_pilot_6.json --event-config configs/eda_apophis_2029_refinement.json --root .
```
