...
class SelectorHoldoutTests(unittest.TestCase):
    ...
    def test_v4_seed_and_split_are_distinct(self):
        documents = _catalogues()
        selected = select_objects(documents, set())
        self.assertTrue(all(item["split"] == "selector_v4_holdout24" for item in selected))

    def test_old_holdout_ids_can_be_excluded(self):
        documents = _catalogues()
        self.assertNotIn("10000", {item["id"] for item in select_objects(documents, {"10000"})})

    def test_download_jobs_are_target_only_forty(self):
        jobs = [{"kind": "horizons_asteroid", "i": i} for i in range(40)] + [{"kind": "planet", "i": i} for i in range(9)]
        with patch("prepare_selector_v4_holdout24.build_jobs", return_value=jobs):
            self.assertEqual(len(build_selector_jobs(Path("."), {})), 40)
