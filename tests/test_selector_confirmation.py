...
import run_selector_confirmation as runner
...
class SelectorV4RunnerTests(unittest.TestCase):
    ...
    def test_dynamic_matrix_choice_rows_do_not_assume_24_objects(self):
        records = []
        for oid in ("2024_AB1", "123"):
            for horizon in runner.HORIZONS:
                for model in ("cheap", "full"):
                    records.append({"object_id": oid, "model_id": model, "horizon_days": horizon,
                                    "feature": {"vector": [0.] * 9},
                                    "record": {"max_position_error_km": .01, "numerical_difference_km": .001}})
        sample = {"objects": [{"id": "2024_AB1"}, {"id": "123"}]}
        with patch.object(runner, "decide", return_value={"model_id": "cheap", "status": "predicted_feasible", "fallback": False}):
            choices, summaries = runner._choice_rows(records, sample, self.config, {}, self.tree, self.v4)
        self.assertEqual(len(choices), 2 * 5 * 3 * 4)
        self.assertTrue(all(row["cases"] == 2 * 5 for row in summaries))

    def test_stratum_summary_covers_all_methods_tolerances_and_body_horizons(self):
        objects = [{"id": "A", "stratum": "tight"}, {"id": "B", "stratum": "control"}]
        choices = [{"object_id": oid, "method": method, "tolerance_km": tol,
                    "horizon_days": horizon, "actual_eligible": True, "warning": False,
                    "max_position_error_km": .01}
                   for oid in ("A", "B") for method in runner.METHODS for tol in runner.TOLERANCES for horizon in runner.HORIZONS]
        summary = runner._stratum_summaries(choices, objects)
        self.assertEqual(len(summary), 2 * 4 * 3)
        self.assertTrue(all(row["cases"] == 5 and row["body_all_horizons"] == 1 for row in summary))
